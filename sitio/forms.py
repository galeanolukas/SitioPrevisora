...
class PagoForm(ModelForm):
    class Meta:
        ...
        fields = ['usuario', 'categoria', 'medio', 'valor', 'recibo_n', 'fecha']
...
class FormRegistro(forms.Form):
    username = forms.CharField(label='USUARIO', min_length=4, max_length=150, widget=forms.TextInput(attrs={'placeholder': '* USUARIO'}))
    email = forms.EmailField(label='CORREO', widget=forms.EmailInput(attrs={'placeholder': '* CORREO ELECTRONICO'}))
    password1 = forms.CharField(label='CONTRASEÑA', widget=forms.PasswordInput(attrs={'placeholder': '* PASSWORD'}),)
    password2 = forms.CharField(label='REPITE CONTRASEÑA', widget=forms.PasswordInput(attrs={'placeholder': '* REPITE PASSWORD'}))
    is_staff = forms.BooleanField(label='ES STAFF')
    is_active = forms.BooleanField(label='ACTIVADO')
